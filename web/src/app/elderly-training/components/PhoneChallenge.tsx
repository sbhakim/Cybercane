...
const ID = "phone";
const POINTS = 100;
const CORRECT_VERDICT: Verdict = "scam";
const ALL_INDICATORS: { key: Indicator; label: string; correct: boolean; help?: string }[] = [
  { key: "mismatch", label: "Caller ID mismatch vs claim", correct: true, help: "Spoofed caller ID." },
  { key: "urgent", label: "Urgent request for money", correct: true, help: "Time pressure lowers defenses." },
  { key: "giftcards", label: "Gift card payment request", correct: true, help: "Untraceable and common scam method." },
  { key: "secrecy", label: "Secrecy request (don't tell parents)", correct: true, help: "Isolation tactic." },
  { key: "callbackmatch", label: "Callback number matches family contact", correct: false },
  { key: "safeword", label: "Caller verified personal safe word", correct: false },
];

const PAYLOAD = "scam|mismatch,urgent,giftcards,secrecy";

export default function PhoneChallenge({
  result,
  onSolved,
  registerRef,
}: {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!exactMatch) return resetFeedbackSoon("Close! Consider the payment and secrecy red flags.");
  ...
  return (
    <div ref={sectionRef} className="rounded-2xl border bg-white shadow-sm dark:bg-slate-900/60 dark:border-slate-800 p-6">
      <h3 className="text-lg font-semibold">Phone/SMS “Grandchild in Trouble”</h3>
      <p className="text-sm text-slate-600 dark:text-slate-300 mt-1">Assess the scenario and mark the red flags.</p>

      <div className="mt-4 rounded-md border p-3 bg-slate-50 dark:bg-slate-800/50 text-sm">
        Caller ID shows <strong>Local Police</strong>, but caller claims: “Grandma, keep it secret, send <strong>$3000 in Apple gift cards</strong> to get me out.”
      </div>
